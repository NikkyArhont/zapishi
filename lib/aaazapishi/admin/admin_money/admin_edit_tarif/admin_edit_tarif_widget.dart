...
import 'admin_edit_tarif_model.dart';
export 'admin_edit_tarif_model.dart';

class AdminEditTarifWidget extends StatefulWidget {
  const AdminEditTarifWidget({
    super.key,
    required this.tarifDoc,
  });

  final TarifRecord? tarifDoc;

  @override
  State<AdminEditTarifWidget> createState() => _AdminEditTarifWidgetState();
}

class _AdminEditTarifWidgetState extends State<AdminEditTarifWidget> {
  late AdminEditTarifModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => AdminEditTarifModel());

    // On component load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.tarifType = widget.tarifDoc?.forWho;
      safeSetState(() {});
    });

    _model.workerAmountTextController1 ??=
        TextEditingController(text: widget.tarifDoc?.title);
    _model.workerAmountFocusNode1 ??= FocusNode();

    _model.workerAmountTextController2 ??=
        TextEditingController(text: widget.tarifDoc?.duration.toString());
    _model.workerAmountFocusNode2 ??= FocusNode();

    _model.workerAmountTextController3 ??=
        TextEditingController(text: widget.tarifDoc?.cost.toString());
    _model.workerAmountFocusNode3 ??= FocusNode();

    _model.workerAmountTextController4 ??=
        TextEditingController(text: widget.tarifDoc?.description);
                ...
                Align(
                  ...
                  child: FFButtonWidget(
                    onPressed: ((_model.workerAmountTextController1.text ==
                        ...
                        ? null
                        : () async {
                            await widget.tarifDoc!.reference
                                .update(createTarifRecordData(
                              title: _model.workerAmountTextController1.text,
